...
Config g_config;
char conf_filename[1024] = "bbserv.conf";
...
bool BulletinBoard::replace_msg(int msg_id, string user, string msg)
{
    ...
    if(begin_pos != -1)
    {
        ...
        //Finally, add this msg_id to complete the replace
...
bool BbServer::InitServer()
{
    ...
    int ret;
    struct sockaddr_in bp_address;
    memset(&bp_address, 0, sizeof(bp_address)); //initialize address
    bp_address.sin_family = AF_INET;
    inet_pton(AF_INET, "127.0.0.1", &bp_address.sin_addr);
    bp_address.sin_port = htons(g_config.bp);

    struct sockaddr_in sp_address;
    memset(&sp_address, 0, sizeof(sp_address)); //initialize address
    sp_address.sin_family = AF_INET;
    inet_pton(AF_INET, "127.0.0.1", &sp_address.sin_addr);
    sp_address.sin_port = htons(g_config.sp);

    m_bp_socketFd = socket(AF_INET, SOCK_STREAM, 0); //create socket
    if (m_bp_socketFd < 0) {
        if(g_config.D) cout << "Server: bp socket error! id:" << m_bp_socketFd << endl;
        return false;
    }
    m_sp_socketFd = socket(AF_INET, SOCK_STREAM, 0); //create socket
    if (m_sp_socketFd < 0) {
        if(g_config.D) cout << "Server: sp socket error! id:" << m_sp_socketFd << endl;
        return false;
    }

    ret = bind(m_bp_socketFd, (struct sockaddr*)&bp_address, sizeof(bp_address)); //bind
    if (ret == -1) {
        if(g_config.D) cout << "Server: bp bind error!" << endl;
        return false;
    }

    ret = bind(m_sp_socketFd, (struct sockaddr*)&sp_address, sizeof(sp_address)); //bind
    if (ret == -1) {
        if(g_config.D) cout << "Server: sp bind error!" << endl;
        return false;
    }

    ret = listen(m_bp_socketFd, 20); //listen
    if (ret == -1) {
        if(g_config.D) cout << "Server: bp listen error!" << endl;
        return false;
    }

    ret = listen(m_sp_socketFd, 20); //listen
    if (ret == -1) {
        if(g_config.D) cout << "Server: sp listen error!" << endl;
        return false;
    }

    s_epollFd = epoll_create(5);
    if (s_epollFd == -1) {
        if(g_config.D) cout << "Server: create epoll error!" << endl;
        return false;
    }
    Addfd(s_epollFd, m_bp_socketFd); //Register for events on sock_fd
    //Addfd(s_epollFd, m_sp_socketFd); //Register for events on sock_fd

    ret = socketpair(PF_UNIX, SOCK_STREAM, 0, s_pipeFd); //Create a pipe using socketpair and register read-write events on s_pipeFd[0]
    if (ret == -1) {
        if(g_config.D) cout << "Server: socketpair error!" << endl;
        return false;
    }
...
void BbServer::InitSignalServer()
{
    //set signal mask
    AddSignal(SIGHUP);
    AddSignal(SIGQUIT);
    AddSignal(SIGTERM); //Terminate the process
    AddSignal(SIGINT); //Keyboard input to interrupt process Ctrl+C
    AddSignal(SIGALRM); //timer expires
}

void BbServer::InitDaemonServer()
{
    if(g_config.d)
    {
        //redirect standard output
        FILE * pFile = fopen("bbserv.log", "w"); //If the file does not exist, create the file first
        fclose(pFile);
        int std_out_fd = open("bbserv.log", O_WRONLY | O_TRUNC);
        dup2(std_out_fd, 1);
        ...
        cout << "in daemon process" << endl;
    }
}

int BbServer::SetNonblocking(int fd)
{
...
void BbServer::Addfd(int epoll_fd, int sock_fd)
{
    epoll_event event;
...
void BbServer::SignalHandler(int sig)
{
    // Keep the original errno and restore after the function is executed to ensure the reentrancy of the function
    int old_errno = errno;
    int msg = sig;
    send(s_pipeFd[1], reinterpret_cast<char *>(&msg), 1 , 0); //Write the signal to the pipe to notify the main loop
    errno = old_errno;
}

void BbServer::AddSignal(int sig)
{
    struct sigaction sa;
    ...
    if (sigaction(sig, &sa, nullptr) == -1) {
        if(g_config.D) cout << "Server: Add signal error!" << endl;
    }
}

void BbServer::HandleSignal(const string &sigMsg) {
    for (auto &item : sigMsg){ //Process the received signals one by one
        switch (item) {
            case SIGHUP:
            case SIGQUIT: {
                CloseAllFds();
                ...
                if(item == SIGQUIT) {
                    //exit process
                    m_serverStop = true; //Safely terminate the server main loop
            ...
            case SIGTERM: {
                cout << "Signal: SIGTERM: Kill" << endl;
                m_serverStop = true; //Safely terminate the server main loop
                break;
            }
            case SIGINT: {
                cout << "Signal: SIGINT: Ctrl+C" << endl;
                m_serverStop = true; //Safely terminate the server main loop
                break;
            }
            case SIGALRM: {
                if(g_config.D) cout << "Signal: SIGINT: Ctrl+C" << endl;
                // Use timeout to mark timed tasks
                m_timeout = true;
                break;
            }
        }
...
void BbServer::TimerHandler() {
    // m_timerList.Tick(); //Call the Tick() function of the ascending timer linked list class to process the tasks due on the linked list
    // m_timerWheel.Tick(); //Call the heartbeat function Tick() of the time wheel to process the tasks due on the linked list
    // alarm(TIMESLOT);  //Signal SIGALRM again

    m_timerHeap.Tick(); //Call the heartbeat function Tick() of the time heap to process the due tasks
    heap_timer *tmp = nullptr;
    // Determine whether a scheduled task is currently in progress, if not, trigger the SIGALRM signal
...
void BbServer::CloseOneClient(int sockfd)
{
    TimerCallBack(&m_user[sockfd]);
    if (m_user[sockfd].timer) {
        // m_timerList.DelTimer(timer);
        // m_timerWheel.DelTimer(timer);
        m_timerHeap.DelTimer(m_user[sockfd].timer);
    }
    s_clientsList.remove(sockfd);
    if(g_config.D) cout << "Server: Client close socket!" << endl;
    if(g_config.D) cout << "Server: Now client number:" << s_clientsList.size() << endl;
}
